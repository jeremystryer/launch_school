...
// On line 22, forEach is called on arguments, the variable that is available in every JavaScript function regardless of whether or not parameters exist for that function. The Array.prototype.forEach() function can
// only be invoked on arrays. However, the arguments object is not an array. It allows for index retrieval of values and length, but it does not have any other array functions available to it.

// One fix is to use the rest operator and an args parameter like so:
...
//   let sum = 0;
//
//   Array.prototype.slice.call(arguments).forEach(atom => {
//     const match   = /([a-zA-Z]+)([0-9]*)/.exec(atom);
//     const element = match[1];
//     const number  = parseInt(match[2]) || 1;
//
