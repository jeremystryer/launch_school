...
// Array.prototype.forEach() will always continue iteration through the entire collection. It is not possible to stop iteration with a break or return statement. Instead, a for loop can be used.

function isReserved(name) {
  for (let index = 0; index < RESERVED_KEYWORDS.length; index += 1) {
    if (name === RESERVED_KEYWORDS[index]) return true;
  }
